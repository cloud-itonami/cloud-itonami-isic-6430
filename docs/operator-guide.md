...
3. Run read-only validation of existing records against this blueprint's
   contracts.
4. Configure the TrustFundGovernor's hold/escalation policy.
5. If deploying alongside `cloud-itonami-isic-6499` (the investment
   actor), agree the transport for upstream capital-call drafts (a
...
- disbursing a distribution or publishing a NAV used for real transactions always requires a human sign-off
