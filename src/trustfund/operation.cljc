(ns trustfund.operation
  "OperationActor -- one trust/fund-vehicle operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (TrustAdmin-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the TrustFundGovernor (:govern) and the
  rollout phase gate (:decide) before anything commits to the SSoT.

  One graph run = one fund-vehicle operation (intake -> advise -> govern
  ...
  decision to a human trustee/fund officer. `:capital-call/issue-notice`
  ALWAYS reaches this node when the governor is clean -- see
  `trustfund.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [trustfund.advisor :as advisor]
...
(defn build
  ...
  `trustfund.store/Store`).
  opts:
    :advisor      -- a `trustfund.advisor/Advisor` (default: mock-advisor)
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) (:stake proposal)
                                          :else :low-confidence))
                        :phase ph
